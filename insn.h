...
public:
  ...
  union ARMValue {
    ...
    ARMValue(const ARMValue& v) {
      mem.base  = v.mem.base;
      mem.index = v.mem.index;
      mem.scale = v.mem.scale;
      mem.disp  = v.mem.disp;
    }
  ...
  union X86Value {
    ...
    X86Value(const X86Value& v) {
      mem.segment = v.mem.segment;
      mem.base    = v.mem.base;
      mem.index   = v.mem.index;
      mem.scale   = v.mem.scale;
      mem.disp    = v.mem.disp;
  ...
  Operand(const Operand& op)
      : type(op.type), size(op.size), x86_value(op.x86_value) {}
...
public:
  ...
  Instruction()
      : id(0), start(0), size(0), addr_size(0), target(0), flags(0),
        invalid(false), privileged(false), trap(false) {}
  Instruction(const Instruction& i)
      : id(i.id), start(i.start), size(i.size), addr_size(i.addr_size),
        target(i.target), flags(i.flags), mnem(i.mnem), op_str(i.op_str),
        operands(i.operands), invalid(i.invalid), privileged(i.privileged),
        trap(i.trap) {}
