...
open_bfd(std::string& fname) {
  ...
  if (!bfd_check_format(bin, bfd_object)) {
    print_err(
        "file '%s' does not look like a binary object (%s), maybe load as raw?",
  ...
  if (bfd_get_flavour(bin) == bfd_target_unknown_flavour) {
    print_err("unrecognized format for binary '%s' (%s)", fname.c_str(),
              bfd_errmsg(bfd_get_error()));
    return NULL;
  }
...
load_dynsym_bfd(bfd* bfd_h, Binary* bin) {
  ...
  if (n < 0) {
    print_err("failed to read dynamic symtab (%s)",
              bfd_errmsg(bfd_get_error()));
    goto fail;
  } else if (n) {
...
load_dynrelocs_bfd(bfd* bfd_h, Binary* bin) {
  ...
  if (relsize < 0) {
    print_err("failed to read dynamic relocations size (%s)",
              bfd_errmsg(bfd_get_error()));
    goto fail;
  }
  ...
  if (nrels < 0) {
    print_err("failed to read dynamic relocations (%s)",
              bfd_errmsg(bfd_get_error()));
    goto fail;
  }
  /* Apply relocations */
...
#define APPLY_RELOC(x)                                                         \
  ((x & ~bfd_howto->dst_mask) |                                                \
   (((x & bfd_howto->src_mask) + relocation) & bfd_howto->dst_mask))
      ...
      case 0:
        bfd_put_8(bfd_h, APPLY_RELOC(bfd_get_8(bfd_h, data)), data);
        break;
      case 1:
        bfd_put_16(bfd_h, APPLY_RELOC(bfd_get_16(bfd_h, data)), data);
        break;
      case 2:
        bfd_put_32(bfd_h, APPLY_RELOC(bfd_get_32(bfd_h, data)), data);
        break;
      case 4:
        bfd_put_64(bfd_h, APPLY_RELOC(bfd_get_64(bfd_h, data)), data);
        break;
      default:
...
load_sections_bfd(bfd* bfd_h, Binary* bin) {
  ...
  for (bfd_sec = bfd_h->sections; bfd_sec; bfd_sec = bfd_sec->next) {
    ...
    if (!bfd_get_section_contents(bfd_h, bfd_sec, sec->bytes, 0, size)) {
      print_err("failed to read section '%s' (%s)", secname,
                bfd_errmsg(bfd_get_error()));
      return -1;
    }
  }
...
load_binary_bfd(std::string& fname, Binary* bin, Binary::BinaryType type) {
  ...
  default:
  fail_arch:
    print_err("unsupported architecture (%s, [%u, %u])",
              bfd_info->printable_name, bfd_info->arch, bfd_info->mach);
    goto fail;
  }
