...
public:
  BB()
      : start(0), end(0), function(NULL), section(NULL), score(0.0),
        alive(false), invalid(false), privileged(false), addrtaken(false),
        padding(false), trap(false) {}
  BB(const BB& bb)
      : start(bb.start), end(bb.end), insns(bb.insns), function(bb.function),
        section(bb.section), score(bb.score), alive(bb.alive),
        invalid(bb.invalid), privileged(bb.privileged), addrtaken(bb.addrtaken),
        padding(bb.padding), trap(bb.trap), ancestors(bb.ancestors),
        targets(bb.targets) {}
