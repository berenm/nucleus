...
unsigned
bb_queue_recursive(DisasmSection* dis, BB* parent, BB** mutants, unsigned n,
                   const unsigned max_mutants) {
  uint64_t target;
...
bb_mutate_recursive(DisasmSection* dis, BB* parent, BB** mutants) {
  ...
  if (!parent) {
    ...
    /* first guess for BBs are the entry point and function symbols if
     * available, or the section start address otherwise */
    ...
    for (i = 0; i < symbols->size(); i++) {
      if ((symbols->at(i).type & Symbol::SYM_TYPE_FUNC) &&
          ((n + 1) < max_mutants) &&
 ...
 **                            dispatch functions                             **
 ******************************************************************************/
const char* strategy_functions[] = {"linear", "recursive", NULL};

const char* strategy_functions_doc[] = {
    /* linear     */ "Linear disassembly",
    /* recursive  */
    "Recursive disassembly (incomplete implementation, not recommended)", NULL};

void* bb_strategy_functions[][4] = {
    {(void*)bb_score_linear, (void*)bb_mutate_linear, (void*)bb_select_linear},
    {(void*)bb_score_recursive, (void*)bb_mutate_recursive,
     (void*)bb_select_recursive},
    {NULL, NULL, NULL}};
...
load_bb_strategy_functions() {
  ...
  if (i >= 0) {
    options.strategy_function.score_function =
        (bb_score_function_t)bb_strategy_functions[i][0];
    options.strategy_function.mutate_function =
        (bb_mutate_function_t)bb_strategy_functions[i][1];
    options.strategy_function.select_function =
        (bb_select_function_t)bb_strategy_functions[i][2];
  } else {
    goto fail;
  }
